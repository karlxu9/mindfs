...
**环境**：沙箱后端（隔离 APPDATA、7998、-no-relayer）+ vite dev（代理临时指 7998，验证后还原）+ 调试 Chrome（CDP 9222）。样本数据：proj-a 内初始化 git repo 并造未提交改动（覆盖 diff 视图）；从主实例复制一份真实丰富会话 jsonl+aux（tool call / 代码块 / 长文本，副本只读）注册进沙箱。截图归档 `verify-ui/`（未跟踪）。

**§4 十项清单结果**（桌面 1440 宽 + 移动 390 宽抽查 ①②③）：

| 项 | 截图 | 判定 |
| --- | --- | --- |
| ① 会话消息流 | 03/04、移动 14 | ✅ 气泡/正文/inline code 全中性+accent，无蓝底 |
| ② 会话列表 | 各图右栏、移动 15 | ✅ 选中态 --menu-active-bg 白系高亮 |
| ③ 文件树+项目列表 | 02、移动 16 | ✅ 侧栏 #171717，层级靠明度 |
| ④ 任务看板卡片 | 02/17 | ✅ 卡片边框细白，操作图标语义色 |
| ⑤ Git diff 查看器 | 06/07 | ✅ 增删行红绿、行内高亮、M/U 徽章正常 |
| ⑥ 定时任务+任务模板对话框 | 09/10 | ✅ 面板 #2a2a2a/#2f2f2f 分层，主按钮 accent |
| ⑦ 命令执行卡片 | 05 | ✅ 命令块底 --mindfs-code-bg #171717 |
| ⑧ 诊断面板 | 08 | ✅ 弹窗明度分层清晰，状态字语义色 |
| ⑨ 补全浮层（/） | 11 | ✅ 菜单底 #2f2f2f；skill 命令名为独立语义紫（豁免观察项，深底可读、不刺眼，保留） |
| ⑩ Toast 三级 | 12（fatal）/13（error） | ✅ 固定彩底白字（rgba 豁免类），主题无关；fatal 常驻+重试正常 |

核对四条纪律：无蓝灰残影、无渐变、层级靠明度可辨、focus/选中可见——全部通过；无需动其余 token。

**AA 对比度抽验**（WCAG 相对亮度计算，11 组关键配对）：10 组 ≥4.5:1（正文级，最高 13.97）；唯一不达标为 danger `#e5484d` on `#212121` = 4.11:1（仅 AA-large）。

**Token 微调（唯一一处，±1 档授权范围内）**：graphite 的 `--danger-color` 由 §3.2 的 `#e5484d` 提亮一档为 `#ec5d5e`（4.83:1 on #212121 / 5.37:1 on #171717），CSS 内注明理由。其余 44 个变量与文档 §3.1 表逐字一致。
